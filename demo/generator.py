...
def generate_mock_data(num_users=1000, num_transactions=5000):
    ...
    with open("users.jsonl", "w") as f:
        for user_id in user_ids:
            f.write(f'{{"id": "{user_id}", "name": "User_{random.randint(1, 10000)}", "email": "user{random.randint(1,10000)}@example.com"}}\n')
    ...
    with open("transactions.json", "w") as f:
        ...
        for i, uid in enumerate(chosen_users):
            if i > 0:
                f.write(",")
            f.write(f'{{"id": "{str(uuid.uuid4())}", "user_id": "{uid}", "amount": {round(random.uniform(5.0, 500.0), 2)}, "timestamp": "2026-03-14T12:00:00Z"}}')
        f.write("]")
